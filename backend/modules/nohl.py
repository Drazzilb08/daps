...
from backend.util.constants import episode_regex, season_regex, year_regex
...
class Nohl(DapsModule):
    ...
    @staticmethod
    def dump_debug_json(
        data_list, media_dict, nohl_data, output_dict, logger: Logger, config
    ):
    ...
    @classmethod
    def process_arr_instances(cls, config, nohl_list, logger):
        ...
        if config.instances:
            instance_index = cls.build_instance_index(
                config.instances, config.instances_config
            )
    ...
    def run(self):
        try:
            ...
            output_dict, data_list, media_dict, nohl_data = self.process_arr_instances(
            ...
            self.handle_messages(final_output, self.logger)
