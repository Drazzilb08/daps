...
def sync_to_plex(
    ...
    tmdb_lookup = {media['tmdb_id']: media for media in media_dict if media.get('tmdb_id') is not None}
    tvdb_lookup = {media['tvdb_id']: media for media in media_dict if media.get('tvdb_id') is not None}
    imdb_lookup = {media['imdb_id']: media for media in media_dict if media.get('imdb_id') is not None}
    fallback_lookup = {(media['normalized_title'], media['year']): media for media in media_dict}

    data_dict: List[Dict] = []

    with progress(library_names, desc="Processing Libraries", unit="items", logger=logger, leave=True) as outer_pbar:
        for library in outer_pbar:
            library_data = plex.library.section(library).all()

            with progress(library_data, desc=f"Syncing labels between {app.instance_name.capitalize()} and {library}", unit="items", logger=logger, leave=True) as inner_pbar:
                for library_item in inner_pbar:
                    ...
                    if ids["tmdb"] and ids["tmdb"].isdigit():
                        ...
                        if media_item:
                            ...
                            plex_id = ids["tmdb"]
                            match_type = f"TMDB Media ID: {media_id} - Plex ID {plex_id}"
                    ...
                    if not media_item and ids["tvdb"] and ids["tvdb"].isdigit():
                        ...
                        if media_item:
                            ...
                            plex_id = ids["tvdb"]
                            match_type = f"TVDB Media ID: {media_id} - Plex ID {plex_id}"
                    ...
                    if not media_item and ids["imdb"]:
                        ...
                        if media_item:
                            ...
                            plex_id = ids["imdb"]
                            match_type = f"IMDB Media ID: {media_id} - Plex ID {plex_id}"
                    ...
                    if media_item:
                        logger.debug(f"Matched '{library_item.title}' ({library_item.year}) using {match_type} lookup to '{media_item['title']}' ({media_item['year']})")
    ...
    try:
        ...
        for mapping in config.mappings:
            ...
            if app_type in ['radarr', 'sonarr'] and app_instance:
                app_config: Optional[Dict] = config.instances_config[app_type].get(app_instance)
                if not app_config:
                    logger.error(f"No config found for {app_type} instance '{app_instance}'")
                    continue

                app = create_arr_client(app_config['url'], app_config['api'], logger)
                if not app or not app.connect_status:
                    logger.error(f"Failed to connect to {app_type} instance {app_instance}")
                    continue

                # Fetch parsed media list from ARR client, excluding episodes for Sonarr
                if hasattr(app, 'instance_type') and app.instance_type.lower() == "sonarr":
            ...
            if plex_instances:
                for mapping_block in plex_instances:
                    ...
                    if plex_instance not in config.instances_config.get('plex', {}):
                        logger.error(f"No Plex instance found for {plex_instance}. Skipping...\n")
                        continue
                    ...
                    if library_names:
                        ...
                        logger.info(f"Syncing labels [{label_str}] from {app_type.capitalize()} instance '{app_instance}' to Plex instance '{plex_instance}'")
                        # Collect changes from sync_to_plex and accumulate in output list
                        data_dict = sync_to_plex(plex, labels, media_dict, app, logger, library_names, config)
                        output.extend(data_dict)
                    else:
                        logger.error(f"No library names provided for {plex_instance}. Skipping...")
                        continue
