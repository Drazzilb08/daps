...
def print_output(output: Dict[str, Dict[str, Any]], logger: Logger) -> None:
    ...
    for instance, instance_data in output.items():
        ...
        total_rename_items = len([v['file_info'] for v in instance_data['data'] if v['file_info']])
        total_folder_rename = len([v['new_path_name'] for v in instance_data['data'] if v['new_path_name']])
...
def get_count_for_instance_type(config: SimpleNamespace, instance_type: str, logger: Logger) -> int:
    """
    ...
    if instance_type == "radarr" and getattr(config, "radarr_count", None):
        logger.debug(f"radarr_count found! overriding count from {config.count} to {config.radarr_count}")
        count = config.radarr_count
    elif instance_type == "sonarr" and getattr(config, "sonarr_count", None):
        logger.debug(f"sonarr_count found! overriding count from {config.count} to {config.sonarr_count}")
...
def process_instance(
        ...
        if media_dict:
            ...
            if not getattr(config, "dry_run", False):
                ...
                if tag_id and getattr(config, "tag_name", None):
                    logger.info(f"Adding tag '{config.tag_name}' to items in {app.instance_name}...")
            ...
            total_renamed = sum(len(i['file_info']) for i in media_dict if i.get('file_info'))
    ...
    logger.info(f"Finished processing {app.instance_name} in {time.time() - instance_start_time:.2f} seconds.")
