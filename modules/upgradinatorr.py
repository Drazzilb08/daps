...
def filter_media(
    ...
    for item in media_dict:
        ...
        if (
            checked_tag_id in item['tags'] or
            ignore_tag_id in item['tags'] or
            not item['monitored'] or
            ...
            logger.debug(f"Skipping {item['title']} ({item['year']}), Reason: {', '.join(reasons)}")
            continue
        ...
        if item['seasons']:
            ...
            for i, season in enumerate(item['seasons']):
                ...
                if len(season["episode_data"]) > 0:
                    monitored_percentage = (monitored_count / len(season['episode_data'])) * 100
                else:
            ...
            for instance_type, instance_data in config.instances_config.items():
                if instance_name in instance_data:
                    ...
                    if app and app.connect_status:
                        output = process_instance(instance_type, instance_entry, app, logger, config)
                        final_output_dict.setdefault(instance_name, {}).update(output or {})
