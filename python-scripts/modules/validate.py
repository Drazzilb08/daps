...
class ValidateInput():
    ...
    def validate_global(self, url, api_key, instance_name, instance_type):
        ...
        if instance_type == "Radarr" or instance_type == "Sonarr" or instance_type == "Plex":
            if not (url.startswith("http://") or url.startswith("https://")):
                self.logger.error(f'\'{instance_name}\' URL must start with \'http://\' or \'https://\'')
            if not api_key:
                self.logger.error(f'API key is required for \'{instance_name}\'')
    ...
    def validate_script(self, logger):
        if self.dry_run not in [True, False]:
            self.dry_run = True
            self.logger.error(f'\'dry_run must be either True or False')
        if self.log_level not in ['debug', 'info', 'critical']:
            ...
            if not self.source_dir:
                self.logger.error(f'\'source_dir is required.')
            if not os.path.exists(self.source_dir):
                logger.error(f"Source directory does not exist: {self.source_dir}")
                sys.exit(1)
            if not self.destination_dir:
                self.logger.error(f'\'destination_dir is required.')
            if not os.path.exists(self.destination_dir):
                logger.error(f"Destination directory does not exist: {self.destination_dir}")
                sys.exit(1)
            if not self.movies_threshold:
                self.logger.error(f'\'movies_threshold is required.')
            if self.movies_threshold:
                try:
                    int(self.movies_threshold)
                except ValueError:
                    self.logger.error(f'\'movies_threshold\' must be an integer.')
            if not self.series_threshold:
                self.logger.error(f'\'series_threshold\' is required.')
            if self.series_threshold:
                try:
                    int(self.series_threshold)
                except ValueError:
                    self.logger.error(f'\'series_threshold\' must be an integer.')
            if not self.collection_threshold:
                self.logger.error(f'\'collection_threshold\' is required.')
            if self.collection_threshold:
                try:
                    int(self.collection_threshold)
                except ValueError:
                    self.logger.error(f'\'collection_threshold\' must be an integer.')
            if not self.action_type:
                self.logger.error(f'\'action_type\' is required.')
            if self.action_type not in ['move', 'copy']:
                self.logger.error(f'\'action_type\' must be either \'move\' or \'copy\'.')
            if self.print_only_renames not in [True, False]:
                self.print_only_renames = False
                self.logger.error(f'\'print_only_renames must be either True or False')
