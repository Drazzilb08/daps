...
export const FormValidator = React.memo(({
   ...
   */
  const validateField = useCallback((fieldKey, value, fieldSchema) => {
    if (!fieldSchema) {
      return null;
    }
    ...
    if (fieldSchema.customValidate && typeof fieldSchema.customValidate === 'function') {
   ...
   */
  const handleFieldValidation = useCallback((fieldKey, value) => {
    if (!validateOnChange) {
      return null;
    }
...
export const useFieldValidation = (fieldKey) => {
  ...
  return {
    ...
    validateField: useCallback((value) => validateField(fieldKey, value), [fieldKey, validateField]),
