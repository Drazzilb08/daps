...
export const SearchCoordinatorProvider = ({ 
   ...
   */
  const getSearchState = useCallback((searchType) => {
   ...
   */
  const setSearchState = useCallback((searchType, updates) => {
  ...
  const addToHistory = useCallback((term, searchType) => {
    ...
    setSearchHistory(prev => {
      ...
      const filtered = prev.filter(item => 
        !(item.term === term && item.searchType === searchType)
      );
   ...
   */
  const getHistory = useCallback((searchType = null, limit = 10) => {
   ...
   */
  const search = useCallback((searchType, term, options = {}) => {
    const {
      immediate = false,
  ...
  }, [searchStates, activeSearchType, debounceDelay, addToHistory, getSearchState, setSearchState]);

  /**
   * Clear search results for specific type
   * @param {string} searchType - Type of search to clear
   */
  const clearSearch = useCallback((searchType) => {
   ...
   */
  const getSuggestions = useCallback((searchType, partialTerm, limit = 5) => {
...
export const useSearch = (searchType, searchFunction) => {
  ...
  const suggestions = useCallback((partialTerm, limit) => {
