...
export const normalizeFieldSchema = (fieldSchema, fieldKey) => {
  ...
  const normalized = {
    ...
    // Default value
    default: fieldSchema.default !== undefined ? fieldSchema.default : getFieldTypeDefaultValue(fieldSchema.type),
    
    // Additional custom properties (passed through as-is)
    ...Object.keys(fieldSchema).reduce((acc, key) => {
      if (!['type', 'label', 'description', 'placeholder', 'required', 'disabled', 'hidden',
            'options', 'multiple', 'maxLength', 'minLength', 'pattern', 'min', 'max', 'step',
            'validation', 'customValidate', 'conditionalOn', 'conditionalValue', 
            'conditionalOperator', 'default'].includes(key)) {
    ...
    if (fieldSchema.conditionalOn) {
      ...
      switch (operator) {
        ...
        case 'falsy':
          ...
          break;
        case 'contains':
          isVisible = Array.isArray(dependentValue) && dependentValue.includes(expectedValue);
          break;
        case 'not_contains':
          isVisible = Array.isArray(dependentValue) && !dependentValue.includes(expectedValue);
          break;
...
export const validateSchema = (schema) => {
  ...
  try {
    ...
    // Validate each field
    Object.entries(schema.fields).forEach(([fieldKey, fieldSchema]) => {
      if (!fieldSchema || (typeof fieldSchema !== 'object' && typeof fieldSchema !== 'string')) {
      ...
      if (fieldSchema.conditionalOn && !schema.fields[fieldSchema.conditionalOn]) {
        errors.push(`Field "${fieldKey}" has conditional dependency on non-existent field "${fieldSchema.conditionalOn}"`);
      }
    });
      ...
      schema.sections.forEach((section, index) => {
        if (section.fields && Array.isArray(section.fields)) {
          section.fields.forEach(fieldKey => {
            if (!schema.fields[fieldKey]) {
              errors.push(`Section ${index} references non-existent field "${fieldKey}"`);
            }
          });
        }
      });
