...
export default function GDriveStatsContent({ data, loading, error, refresh }) {
    const [sort, setSort] = useState('files-desc');
    ...
    let totalFiles = arr.reduce((sum, s) => sum + (s.file_count ?? 0), 0);
    let totalSize = arr.reduce((sum, s) => sum + (s.size_bytes ?? 0), 0);
        ...
        <>
            <div className="control-row">
                ...
                <TooltipFactory
                    ...
                    text="Refresh Gdrive Statistics"
                    show={refreshTip}
                    position="top"
                />
            </div>
