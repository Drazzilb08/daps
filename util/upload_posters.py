...
class PosterUploader:
                    ...
                    if plex_client.is_connected():
                        ...
                        for source, asset_id in all_ids:
                            ...
                            if not asset:
                                ...
                                continue
                            assets.append(asset)
                        movie_index, show_index, collection_index = self._build_indexes(
                            plex_media_cache
                        )
                        ...
                        if updated:
                            self.logger.info("[POSTER SYNC] Uploaded:")
                            for x in updated:
                                self.logger.info(f"  - {x}")
                        if skipped:
                            self.logger.info(
                                "[POSTER SYNC] Skipped (unchanged/dry run):"
                            )
                            for x in skipped:
                                self.logger.info(f"  - {x}")
                        if failed:
                            self.logger.info("[POSTER SYNC] Failed:")
                            for x in failed:
                                self.logger.info(f"  - {x}")
                    else:
                        self.logger.warning(
                            f"Skipping sync for {instance_name} (not connected)"
                        )
                else:
                    self.logger.info(f"Skipping sync for {instance_name} (not enabled)")
    ...
    def _sync_series(
        ...
        series_records = [
            a
            for a in records
            if a.get("asset_type") == "show" and a.get("matched") == 1
        ]
        ...
        with progress(
            ...
            for record in bar:
                ...
                if upload_ok:
                    if self.has_overlay(matched_entry) and not season_number:
                        plex_client.remove_label(matched_entry, "Overlay", dry_run)
