...
if (bytes < 1024) return bytes + " B" ; let kb=bytes / 1024; if (kb < 1024) return kb.toFixed(1) + " KB" ; let mb=kb / 1024; if (mb < 1024) return mb.toFixed(1) + " MB" ; return (mb / 1024).toFixed(2) + " GB" ; } function highlight(str, term) { if (!term) return str; const regex=new RegExp(`(${term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&' )})`, 'gi' ); return str.replace(regex, `<span class="highlight">$1</span>`);
    }
    ...
    let nameToLoc = {};
    gdriveLocations.forEach(g => { nameToLoc[g.name] = g.location; });
            ...
            `<li class="img-preview-link">
                ...
                <button class="copy-btn" title="Copy filename" onclick="event.stopPropagation(); copyToClipboard(this, '${f}'); return false;">
                    <span class="copy-btn-default">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 2px;">
    ...
    gdriveFiles = [];
    assetsFiles = [];
    gdriveTotals = { files: 0, size: 0 };
    assetsTotals = { files: 0, size: 0 };

    const config = await fetchConfig();
    gdriveLocations = (config.sync_gdrive && config.sync_gdrive.gdrive_list || []).map(g => ({ name: g.name, location: g.location }));
    ...
    (s.files || []).forEach(f => gdriveFiles.push({ file: f, name: s.name }));
    });
