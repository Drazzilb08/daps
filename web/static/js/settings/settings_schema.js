...
export const SETTINGS_SCHEMA = [
  ...
  {
    key: "poster_renamerr", label: "Poster Renamerr",
    fields: [
      { key: "log_level", label: "Log Level", type: "dropdown", options: ["debug", "info"], required: true, description: "Set the logging verbosity for poster renaming." },
      { key: "dry_run", label: "Dry Run", type: "check_box", description: "Simulate actions without making changes." },
      { key: "sync_posters", label: "Sync Posters", type: "check_box", description: "Enable to synchronize posters between source and destination." },
      { key: "action_type", label: "Action Type", type: "dropdown", options: ["copy", "move", "hardlink", "symlink"], required: true, description: "Select the file operation to use for renaming posters." },
      { key: "asset_folders", label: "Asset Folders", type: "check_box", description: "Enable to use asset folders for organizing posters." },
      { key: "print_only_renames", label: "Print Only Renames", type: "check_box", description: "Only print renaming actions without performing them." },
      { key: "run_border_replacerr", label: "Run Border Replacerr", type: "check_box", description: "Enable automatic border replacement during renaming." },
      { key: "incremental_border_replacerr", label: "Incremental Border Replacerr", type: "check_box", description: "Run border replacerr incrementally with each operation." },
      { key: "run_cleanarr", label: "Run Cleanarr", type: "check_box", description: "Enable to run Cleanarr after renaming." },
  ...
  {
    key: "border_replacerr", label: "Border Replacerr",
    fields: [
  ...
  {
    key: "upgradinatorr", label: "Upgradinatorr",
    fields: [
      ...
      { key: "dry_run", label: "Dry Run", type: "check_box", description: "Simulate upgrade actions without making changes." },
      {
        ...
        fields: [
          ...
          { key: "count", label: "Count", type: "number", required: true, description: "Number of items to upgrade per run." },
  ...
  {
    key: "renameinatorr", label: "Renameinatorr",
    fields: [
      { key: "log_level", label: "Log Level", type: "dropdown", options: ["info", "debug"], required: true, description: "Set the logging verbosity for renameinatorr." },
      { key: "dry_run", label: "Dry Run", type: "check_box", description: "Simulate renaming without making changes." },
      { key: "rename_folders", label: "Rename Folders", type: "check_box", description: "Enable to rename folders as well as files." },
      { key: "count", label: "Count", type: "number", description: "Number of items to rename per operation." },
      { key: "radarr_count", label: "Radarr Count", type: "number", description: "Number of Radarr items to process per run." },
      { key: "sonarr_count", label: "Sonarr Count", type: "number", description: "Number of Sonarr items to process per run." },
      { key: "tag_name", label: "Tag Name", type: "text", description: "Tag name to filter items for renaming." },
      { key: "enable_batching", label: "Enable Batching", type: "check_box", description: "Enable batch processing for renaming." },
      { key: "instances", label: "Instances", type: "instances", required: true, instance_types: ["radarr", "sonarr"], description: "List of Radarr and Sonarr instances to rename." },
    ],
  },
  ...
  {
    key: "health_checkarr", label: "Health Checkarr",
    fields: [
      { key: "log_level", label: "Log Level", type: "dropdown", options: ["info", "debug"], required: true, description: "Set the logging verbosity for health checks." },
      { key: "dry_run", label: "Dry Run", type: "check_box", description: "Simulate health checks without making changes." },
      { key: "instances", label: "Instances", type: "instances", required: true, instance_types: ["radarr", "sonarr"], description: "Instances to run health checks on." },
    ],
  },

  {
    key: "jduparr", label: "Jduparr",
    fields: [
...

  {
    ...
    fields: [
      { key: "log_level", label: "Log Level", type: "dropdown", options: ["debug", "info"], required: true, description: "Set the logging verbosity for general settings." },
      { key: "update_notifications", label: "Update Notifications", type: "check_box", description: "Enable notifications for available updates." },
    ],
  },
  {
    key: "user_interface", label: "User Interface",
    fields: [
      { key: "theme", label: "Theme", type: "dropdown", options: ["dark", "light"], required: true, description: "Choose the UI theme (dark or light)." },
    ],
  },
];
...
export const SETTINGS_MODULES = [
  ...
  { name: 'Renameinatorr', key: 'renameinatorr', description: 'Send rename requests to Sonarr/Radarr instances.' },
  { name: 'Upgradinatorr', key: 'upgradinatorr', description: 'Send automatic search requests to Radarr/Sonarr instances.' },
  { name: 'Nohl', key: 'nohl', description: 'Find items in your media collection that do not have hardlinks and send requests to Radarr/Sonarr to handle them' },
  { name: 'Labelarr', key: 'labelarr', description: 'Sync labels between Radarr/Sonarr -> Plex instances.' },
  { name: 'Health Checkarr', key: 'health_checkarr', description: 'Remove Radarr/Sonarr entries that are no longer in sync with TMDb/TVDb' },
